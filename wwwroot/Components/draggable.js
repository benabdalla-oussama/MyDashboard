  ...
  align-items: center;
  justify-content: center" v-for="handle in handles" v-if="resizable"  class="handle" :key="handle" :class="'handle-' + handle" :style="{display: enabled ? 'block' : 'none'}"
              v-on:mousedown.stop.prevent="handleDown(handle, $event)" v-on:touchstart.stop.prevent="handleDown(handle, $event)" ></div>
        <slot></slot>
        </div > `,
